        ...
        if (!empty($couponCode)) {
            ...
            if ($row_coupon = $result_coupon->fetch_assoc()) {
                ...
                if ($row_coupon['status'] == 1) {
                    ...
                    // Insert coupon usage data into update_coupon table
                    $order_id = $_SESSION['order_id'];
                    $sql_insert_coupon = "INSERT INTO update_coupon (order_id, coupon_code, coupon_price) VALUES (?, ?, ?)";
                    $stmt_insert_coupon = $conn->prepare($sql_insert_coupon);
                    $stmt_insert_coupon->bind_param("isd", $order_id, $couponCode, $discount);
                    $stmt_insert_coupon->execute();
                }
            }
        }
    }
